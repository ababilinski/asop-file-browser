...
/// Prefer the resource location used by a signed macOS app bundle. The
/// generated SwiftPM accessor remains the fallback for `swift run` and tests.
private let localizationBundle: Bundle = {
    if let resources = Bundle.main.resourceURL,
       let bundle = Bundle(url: resources.appendingPathComponent("MTPKit_MTPKit.bundle")) {
        return bundle
    }
    return .module
}()
...
func L(_ key: String, _ args: CVarArg...) -> String {
    let fmt = localizationBundle.localizedString(forKey: key, value: key, table: nil)
